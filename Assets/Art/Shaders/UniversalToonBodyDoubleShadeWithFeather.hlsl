...
////// Lighting:
              ...
              //
                ...
                UTS_LIGHT_LOOP_BEGIN(pixelLightCount - MAINLIGHT_IS_MAINLIGHT)
                    ...
                    if (iLight != i.mainLightID)
#endif         //这里不能调换if和 #endif 的顺序，现在的逻辑是不管 USE_FORWARD_PLUS是否被定义，都用不同的iLight的值来执行if，如果调换顺序，那么只有在USE_FORWARD_PLUS被定义的情况下才会执行if语句，逻辑发生了变化，
               //很高深莫测（也就是并不是会报错，而是逻辑会发生变化）
                    {
